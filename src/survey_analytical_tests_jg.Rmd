...
dependent variable = support (Q2)
independent variables = familiarity (Q10) & perception (Q13)


```{r regression table }
# create data frame with Q10 and Q13

ordinal_log_reg_4_data <- survey_data_tidy_sbv_long_filtered %>% 
  filter(response_type != 3) %>% #filter out self-collected data
  filter(question == "q2" | 
         question == "q4" |
         question == "q6" |
...
```{r create ordinal logistic model}
#use polr to create a logit model to understand how familiarity influences expansion
m4 <- polr(factor(q2) ~q10 + q13, data=ordinal_log_reg_4, Hess =TRUE)
summary(m4)
```
