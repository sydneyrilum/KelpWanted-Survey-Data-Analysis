...
library(data.table)
library(raster) #for zipcodeR to work?
library(zipcodeR) # for zipcode assignment by county, need to install extra packages if working on a windows (needed raster and dplyr loaded)
library(dplyr)
```
...
survey_data_qrcode_tidy <- survey_data_qrcode %>%
  ...
  dplyr::select(-recipient_last_name, # remove empty/unnecessary columns
...
survey_data_tidy_too <- survey_data_tidy %>% 
  pivot_longer(c(q12, q13), 
               names_to = "question", 
               values_to = "answer") %>% 
  dplyr::select(response_id, response_type, question, answer)
...
us_zipcodes <- read_sf(here("data", "zipcode_tabulation_areas_2020", "tl_2021_us_zcta520.shp")) %>% #this .shp is not in the rproj
  clean_names()
...
```{r}
# Creating a dataframe that is only zipcodes in Ventura and Santa Barbara Counites
## Not run:
...
santabarbara_co__zip_full <- 
 search_county("Santa Barbara" , "CA")

santabarbara_zipcodes<- 
  santabarbara_co__zip_full[,c("zipcode" ,"county", "lat", "lng")]

sb_ventura_co_zip <- 
  rbind(ventura_zipcodes, santabarbara_zipcodes)

```

```{r}
#select survery results from only Ventura and Santa Barbara Counties
survey_data_tidy_sb_ventura <-
 merge(survey_data_tidy, sb_ventura_co_zip, by.x = "q37", by.y = "zipcode")
