...
weights <- read.csv(here("data", "census", "population_sample_margin_weights_adjusted.csv")) %>% 
...
survey_weights <- merge(survey, weights,
                        by.x = c("age_group", "sex"),
                        by.y = c("age_group", "sex"))

# convert data frame into a data table
survey_data_tidy_sbv_long_filtered <- as.data.table(survey_weights)
...
survey_data_tidy_sbv_long_filtered <- survey_weights %>% 
  melt(id.vars = c("response_id", "response_type", "finished", "duration_in_seconds", "county", "age", "age_group", "sex",
