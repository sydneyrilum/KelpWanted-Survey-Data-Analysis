...
## used in final report!! (for environmental impacts)

```{r}
# trust in scientists (Q27_2)
scientists_trust <- survey_data_tidy_sbv_long_filtered %>% 
  select(question, answer, weight) %>% 
  filter(question == "q27_2" | 
           question == "q27_3") %>% 
  mutate(question = case_when(
    question == "q27_2" ~ "I trust scientists who study how we use the environment.",
    question == "q27_3" ~ "I am concerned about climate change."
  )) %>% 
  drop_na() %>% 
  group_by(question, answer) %>% 
  summarize(weight_count = round(sum(weight),)) %>% 
...
# Specify categorical order of questions and answers
scientists_trust$answer <- factor(scientists_trust$answer, levels = c("Don't know", "Strongly Disagree", "Disagree","Neutral","Agree","Strongly Agree"))

# percent stacked barchart
ggplot(data = scientists_trust, 
       aes(fill = forcats::fct_rev(answer),
           y = proportion,
           x = question)) +
  geom_bar(position = "stack",
  ...
  labs(y = "Proportion of Responses",
       x = "",
       title = "To what extent do you agree with each of the following statements?") +
  theme_minimal() +
  theme(legend.title = element_blank(),
        legend.position = "right",
        axis.line.y = element_line(color = "black", size = 0.5),
...
ggsave(filename = file.path(here("graphs","scientists_trust.png")))
```

```{r}
# envi impacts
envi_impacts <- survey_data_tidy_sbv_long_filtered %>% 
  select(question, answer, weight) %>% 
  filter(question == "q23_1" | 
           question == "q23_3" |
           question == "q26_2" |
           question == "q26_3") %>% 
  mutate(question = case_when(
    question == "q23_1" ~ "Seaweed aquaculture has negative impacts on marine ecosystems.",
    question == "q23_3" ~ "Seaweed aquaculture infrastructure poses a risk to marine life.",
    question == "q26_2" ~ "Seaweed aquaculture has fewer risks than shellfish and fish aquaculture.",
    question == "q26_3" ~ "The benefits of seaweed aquaculture outweigh the potential risks."
  )) %>% 
  drop_na() %>% 
  group_by(question, answer) %>% 
  summarize(weight_count = round(sum(weight),)) %>% 
...
# Specify categorical order of questions and answers
envi_impacts$answer <- factor(envi_impacts$answer, levels = c("Don't know", "Strongly Disagree", "Disagree","Neutral","Agree","Strongly Agree"))

envi_impacts$question <- factor(envi_impacts$question, levels = c("Seaweed aquaculture has negative impacts on marine ecosystems.","Seaweed aquaculture infrastructure poses a risk to marine life.","Seaweed aquaculture has fewer risks than shellfish and fish aquaculture.","The benefits of seaweed aquaculture outweigh the potential risks."))

# percent stacked barchart
ggplot(data = envi_impacts, 
       aes(fill = forcats::fct_rev(answer),
           y = proportion,
           x = question)) +
  geom_bar(position = "stack",
...
ggsave(filename = file.path(here("graphs","envi_impacts.png")),
       width = 10, height = 7)
```
...
econ_impacts <- survey_data_tidy_sbv_long_filtered %>% 
  ...
  filter(question == "q24_4" | 
           ...
           question == "q25_5") %>% 
  mutate(question = case_when(
    question == "q24_4" ~ "Aquaculture will be an important food supply as climate change progresses.",
    question == "q25_1" ~ "Seaweed aquaculture has a positive impact on the local economy",
    question == "q25_2" ~ "Seaweed aquaculture lowers the price of seafood.",
    question == "q25_4" ~ "Seaweed aquaculture interferes with commercial and recreational fishing.",
    question == "q25_5" ~ "Seaweed aquaculture interferes with tourism/recreation.")) %>%
  drop_na(answer) %>% 
  group_by(question, answer) %>% 
  summarize(weight_count = round(sum(weight),)) %>% 
  mutate(proportion = (weight_count/sum(weight_count)))
...
# Specify categorical order of questions and answers
econ_impacts$answer <- factor(econ_impacts$answer, levels = c("Don't know", "Strongly Disagree", "Disagree","Neutral","Agree","Strongly Agree"))

econ_impacts$question <- factor(econ_impacts$question, levels = c("Aquaculture will be an important food supply as climate change progresses.","Seaweed aquaculture has a positive impact on the local economy","Seaweed aquaculture lowers the price of seafood.", "Seaweed aquaculture interferes with commercial and recreational fishing.","Seaweed aquaculture interferes with tourism/recreation."))
...
ggsave(filename = file.path(here("graphs","econ_impacts.png")),
       width = 12, height = 7)
```
...
comm_data <- survey_data_tidy_sbv_filtered %>% 
  select("q29_1", "q29_2", "q29_3", "q29_4", "q29_5", "q29_6", "weight") %>% 
  mutate_if(is.character,as.numeric) %>% 
  summarize_all(mean, na.rm = TRUE) # calculate mean rank score for each answer choice
