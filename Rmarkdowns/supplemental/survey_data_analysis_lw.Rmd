...
```
```{r}
# alternative data frame for column
perception_aq_combined_col <- survey_data_tidy_long %>% 
  select(question, answer) %>% 
  filter(question == "q12" | question == "q13") %>% 
  mutate(question = case_when( 
    question == "q12" ~ "aquaculture",
  ...
  mutate(number = case_when( 
    ...
    answer == "Neutral" ~ 2,
    answer == "Positive" ~ 3,
    answer == "Very Positive" ~ 3,
    answer == "Unsure/Not familiar" ~ 4)) %>%
  #replace_with_na(replace = list(number = -999)) %>% 
  drop_na() %>% 
  group_by(question, answer, number) %>% 
  summarize(count = n()) %>% 
...
# Specify categorical order of questions and answers
perception_aq_combined_col$answer <- factor(perception_aq_stacked_bar$answer, levels = c(
                                                                                        "Negative", 
                                                                                        "Neutral",
                                                                                        "Positive",
                                                                                        "Unsure/Not familiar"))
perception_aq_combined_col$question <- factor(perception_aq_stacked_bar$question, levels = c("seaweed aquaculture",
                                                                                            "aquaculture"))
```

```{r}
# combined column graphs
ggplot(data = perception_aq_combined_col, 
       aes(fill = forcats::fct_rev(answer),
           x = proportion,
           y = question)) +
